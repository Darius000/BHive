...
namespace BHive
{
	...
	ViewportPanel::ViewportPanel(const std::string& label, Ref<Viewport> viewport)
		:ImGuiPanel(label, 0), m_Viewport(viewport)
	{
	...
	void ViewportPanel::OnRenderWindow()
	{
		...
		uint32 textureID = m_Viewport->m_SceneFramebuffer->GetColorAttachmentRendererID();
		uint32 textureID2 = m_Viewport->m_Framebuffer->GetColorAttachmentRendererID();

		ImGui::Text("Quad Scene");
		ImGui::Image((void*)textureID, ImVec2(m_ViewportSize.x, m_ViewportSize.y), ImVec2(0, 1), ImVec2(1, 0));

		ImGui::Text("Original Scene");
		ImGui::Image((void*)textureID2, ImVec2(m_ViewportSize.x, m_ViewportSize.y), ImVec2(0, 1), ImVec2(1, 0));
	...
	void ViewportPanel::OnMouseScrolled(MouseScrolledEvent& e)
	{
		...
		m_CameraPosition += FVector3(m_ZoomLevel) * m_Viewport->m_Camera.GetComponent<TransformComponent>().m_Transform.GetForward();
