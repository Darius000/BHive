...
#include "Platforms/Opengl/OpenGLMultiSampleFrameBuffer.h"

namespace BHive
{

	Ref<FrameBuffer> FrameBuffer::Create(const std::string& name, const FrameBufferSpecification& spec)
	{
		switch (Renderer::GetAPI())
		{
		case RendererAPI::API::None: BH_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
		case RendererAPI::API::OpenGL: return Make_Ref<OpenglFramebuffer>(name, spec);
	...
	Ref<BHive::FrameBuffer> FrameBuffer::CreateMultiSample(const std::string& name, const FrameBufferSpecification& spec)
	{
		switch (Renderer::GetAPI())
		{
		case RendererAPI::API::None: BH_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
		case RendererAPI::API::OpenGL: return Make_Ref<OpenGLMultiSampleFrameBuffer>(name, spec);
		case RendererAPI::API::DirectX: break;
		}
